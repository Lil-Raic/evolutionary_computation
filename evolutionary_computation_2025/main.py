...
"""
    ...
    # Example test run for Task 4
    dim_test = 10
    max_fes_test = 3000 * dim_test
    problem_test = Sphere(dim_test)

    print(f"Executing COA on {problem_test.name} (D={dim_test})...")

    # NP=30 is the recommended population size for COA
    coa = CrayfishOptimizationAlgorithm(pop_size=30)

    # Execute the new algorithm
    best_coa_fit = coa.execute(problem_test, max_fes_test)

    print(f"COA {problem_test.name}({dim_test}) best fitness: {best_coa_fit}")
    print("\nTo generate all required .txt files for Task 4.1 submission,")
    print("please run: 'python run_coa_experiment.py'")